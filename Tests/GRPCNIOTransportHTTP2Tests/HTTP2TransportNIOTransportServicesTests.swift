...
final class HTTP2TransportNIOTransportServicesTests: XCTestCase {
  ...
  func testCustomTransportSecurityCallbackInvoked() async throws {
    let server = GRPCNIOTransportCore.HTTP2ServerTransport.TransportServices(
      address: .ipv4(host: "127.0.0.1", port: 0),
      transportSecurity: .plaintext
    )

    // Using "Tracker" instead of an XCTestExpectation because expectations can't be used with
    // @Sendable closures (such as the one we pass to group.addTask below).
    ...
    try await withThrowingDiscardingTaskGroup { group in
      ...
      group.addTask {
        ...
        let customSecurity = HTTP2ClientTransport.TransportServices.TransportSecurity.customSecure
        ...
        let clientTransport = try HTTP2ClientTransport.TransportServices(
          target: .ipv4(address: "127.0.0.1", port: ipv4Address.port),
          transportSecurity: customSecurity,
          config: config
        )
