...
private struct LoopbackListenerFactory: HTTP2ServerTransport.ListenerFactory {
  ...
  func testCustomTransportSecurityCallbackInvoked() async throws {
    let server = GRPCNIOTransportCore.HTTP2ServerTransport.Posix(
      address: .ipv4(host: "127.0.0.1", port: 0),
      transportSecurity: .plaintext
    )

    // Using "Tracker" instead of an XCTestExpectation because expectations can't be used with
    // @Sendable closures (such as the one we pass to group.addTask below). See
    ...
    try await withThrowingDiscardingTaskGroup { group in
      ...
      group.addTask {
        ...
        let customSecurity = HTTP2ClientTransport.Posix.TransportSecurity.customSecure
